...
🔑 لینک سرور : <code>$link</code>":"");

if($botState['subLinkState'] == "on") $acc_text .= "

🔋 مشخصات حجم مصرفی و تاریخ انقضاء:  $botUrl"."search.php?id=".$uniqid."


🌐 لینک سابسکریپشن : <code>$subLink</code>
...
if($data=="getTestAccount"){
    ...
    if($respd->num_rows > 0){
    	...
    	$keyboard[] = [['text' => $buttonValues['back_to_main'], 'callback_data' => "mainMenu"]];
        editText($message_id,"لطفا نوع سرور مد نظر خود را انتخاب کنید؛
لطفا توجه کنید یکبار مجاز به دریافت تست هستید پس با دقت انتخاب کنید. البته اگر مایل به تست مجدد از نوع دیگر باشید میتونید از تیکت درخواست بدید", json_encode(['inline_keyboard'=>$keyboard]), "HTML");
...
if($data == 'reciveApplications') {
    ...
    $keyboard = array_chunk($keyboard,1); 
    editText($message_id, "
🔸تمامی برنامه هایی که واستون قرار دادیم تست شده و لینک های دانلود بدون فیلتر و آپدیت هستند. براحتی میتوانید با اینترنت عادی دانلود و نصب کنید
📌 آموزش اتصال هر یک از برنامه ها بصورت ویدیویی و تصویری در کانال آموزش ما قرار دارند:
@cconfig_v2ray_FAQ
